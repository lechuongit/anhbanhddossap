...
def request_post_attack():
    ...
    while True:
        try:
            ...
            if not isKilled():
                print("[" + yl + "!" + wi + "] " + yl + "BẢNH ĐẤM THẲNG VÀO:" + wi + " Attack To Connect to Target [" + rd + target + wi + "]" + yl + " Maybe " + rd + "Down\n" + wi, end='\r')
            else:
                break
            if hasattr(err, 'errno'):
                if err.errno == 24:
    ...
    if started:
        ...
        while True:
            if len(THREADS) == fin_threads:
                break
    print(wi + "[" + gr + "*" + wi + "] CẢM ƠN ĐÃ DÙNG BẢNH DDOS V1.0.3 :)")
    if started:
...
╚═══════ ೋღ ✿ ღೋ ═══════╝
            ...
            [---]   by:>\033[1;32m WELCOME BẢNH 2 \033[1;37m   [---]\033[1;32m
            =-------=-=-=-=-=-=-=-=-=-------=
"""
started = False
...
Usage: python3 ./anhbanhddossap.py [OPTIONS...]
-------------
OPTIONS:
       |
    |--------
...
os.system("cls || clear")
print("\n" + banner)
print(wi + "[" + gr + "*" + wi + "]" + yl + "-=-=-=-=-=-=-= " + gr + "ĐẤN VÀO IP :  (" + yl + f"{target}:{port}" + gr + ")" + yl + " -=-=-=-=-=-=-=" + wi + "[" + gr + "*" + wi + "]")
print("[" + yl + "I" + wi + "] Infinity [ " + yl + attack.upper() + wi + " ] requests started On Target...")
print("[" + yl + "I" + wi + "] Nhấn " + gr + "' Ctrl+C ' " + wi + "Để Dừng Tấn Công")
